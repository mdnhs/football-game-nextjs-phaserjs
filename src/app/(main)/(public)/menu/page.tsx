...
export default function MenuPage() {
  ...
  useEffect(() => {
    if (ready && player?.displayName) setPlayerName(player.displayName);
  }, [ready, player?.displayName, setPlayerName]);
      ...
      <div className='relative z-10 flex flex-1 flex-col items-center justify-between gap-5 overflow-y-auto px-6 py-4'>
        <div className='flex w-full max-w-sm flex-col items-center gap-4 pt-2'>
          ...
          <h1 className='text-center text-3xl font-black tracking-tight text-white'>
            PENALTY<span className='ml-2 text-green-400'>SHOWDOWN</span>
          </h1>
          <p className='text-center text-sm text-gray-400'>5 shots · Score goals · Top the board</p>
        </div>
        ...
        <div className='flex w-full max-w-sm flex-col gap-4'>
          <div className='grid grid-cols-2 gap-3'>
            ...
            />
            <Stat label='Best score' value={rank?.score ?? '—'} accent='gold' />
          </div>
          ...
          {rank?.rank && (
            <div className='flex items-center justify-center gap-2 rounded-2xl border border-yellow-400/20 bg-yellow-400/10 px-4 py-3 text-center'>
              ...
              <p className='text-xs font-semibold text-yellow-200/90'>
                Campaign rank <span className='font-black text-yellow-400'>#{rank.rank}</span>
              </p>
            </div>
          )}
...
function Stat({ label, value, accent }: { label: string; value: string | number; accent: 'green' | 'gold' | 'red' }) {
  const color = accent === 'green' ? 'text-green-400' : accent === 'gold' ? 'text-yellow-400' : 'text-red-400';
  return (
