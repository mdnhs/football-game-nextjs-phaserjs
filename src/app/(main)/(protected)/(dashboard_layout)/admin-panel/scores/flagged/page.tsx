...
export default function FlaggedScoresPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api<Paginated<FlaggedScore>>(`/api/admin/scores/flagged?page=${page}&limit=${limit}`);
      setData(res);
    } catch (err) {
          ...
          <Table>
            ...
            <TableBody>
              {loading && !data
                ...
                : data?.data.map((s) => (
                    <TableRow key={s.id}>
                      <TableCell className='font-medium'>{s.players?.display_name ?? '—'}</TableCell>
                      <TableCell className='font-mono text-sm'>{s.players?.phone ?? '—'}</TableCell>
                      <TableCell className='text-right'>
                        <Badge variant='destructive'>{s.total_score}</Badge>
                      </TableCell>
                      <TableCell className='text-right'>{s.goals}</TableCell>
                      <TableCell className='text-muted-foreground text-sm'>{fmtDate(s.played_at)}</TableCell>
                      <TableCell className='text-right'>
              ...
              <TableBody>
                {selected.shot_log.map((s) => (
                  <TableRow key={s.shotIndex}>
                    ...
                    <TableCell>
                      <Badge variant={s.result === 'goal' ? 'default' : s.result === 'saved' ? 'secondary' : 'outline'}>
                        {s.result}
                      </Badge>
                    </TableCell>
                    <TableCell className='text-right font-mono'>{s.timing.toFixed(2)}</TableCell>
                    <TableCell className='text-right font-mono'>{s.power.toFixed(2)}</TableCell>
                    <TableCell className='text-right font-mono'>{s.durationMs}ms</TableCell>
