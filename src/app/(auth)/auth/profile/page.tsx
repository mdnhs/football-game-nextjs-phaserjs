...
import { completeProfileSchema } from '@/features/auth/validations/auth-schema';
...
export default function CompleteProfilePage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    const parsed = completeProfileSchema.safeParse({ displayName });
    if (!parsed.success) {
      setError(parsed.error.issues[0]?.message ?? 'Invalid name');
      return;
    }

    const response = await completeProfileMutation.mutateAsync({ displayName: parsed.data.displayName });
  ...
  return (
    <main className='relative flex h-screen w-full flex-col items-center justify-center overflow-hidden bg-[#000814] px-6'>
      ...
      <form onSubmit={onSubmit} className='relative z-10 flex w-full max-w-sm flex-col items-center gap-6'>
        <div className='flex flex-col items-center gap-3 text-center'>
          ...
          <h1 className='text-3xl font-black tracking-tight text-white'>Create profile</h1>
          <p className='max-w-64 text-sm text-gray-400'>Pick the display name that will appear on the leaderboard.</p>
        </div>
        ...
        <div className='flex w-full flex-col gap-3 rounded-2xl border border-white/10 bg-[#071225]/80 p-4 shadow-2xl shadow-black/20 backdrop-blur-sm'>
          <label className='text-xs font-semibold tracking-widest text-gray-400 uppercase'>Display name</label>
          ...
          <Button type='submit' disabled={saving} className='w-full' size='lg' variant='primary'>
