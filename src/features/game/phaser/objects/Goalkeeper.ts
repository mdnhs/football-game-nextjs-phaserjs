...
export class Goalkeeper {
  ...
  private setExpression(type: Expression) {
    ...
    const drawBrow = (g: Phaser.GameObjects.Graphics, cx: number, cy: number, rotDeg: number) => {
      g.save();
  ...
  private scheduleBlink() {
    this.scene.time.delayedCall(2500 + Math.random() * 2500, () => {
  ...
  reactToShot({
    ...
    if (willSave) {
      ...
      goesRight = Math.random() < predictChance ? direction.x > 0 : direction.x <= 0;
    }
  ...
  private spawnDust(x: number, y: number) {
    for (let i = 0; i < 8; i++) {
      ...
      const dot = this.scene.add.circle(x + offX, y, Phaser.Math.Between(3, 6), 0xddd0b8, 0.65).setDepth(11);
      this.scene.tweens.add({
        targets: dot,
        x: dot.x + offX * 2,
  ...
  private spawnSparkles(count = 8) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const startY = this.startY - 110 + (Math.random() - 0.5) * 50;
