...
import type { AimDirection, ShotLogEntry, ShotResult, WindInfo } from '@/features/game/types';
...
export class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    if (!usePhotoStadium) {
      ...
      stadiumFx.fillTriangle(width * 0.02, 0, width * 0.27, 0, width * 0.56, height);
      stadiumFx.fillTriangle(width * 0.73, 0, width * 0.98, 0, width * 0.44, height);
      stadiumFx.fillStyle(0xfff8dd, 0.028);
      stadiumFx.fillTriangle(width * 0.16, 0, width * 0.38, 0, width * 0.5, height);
      stadiumFx.fillTriangle(width * 0.62, 0, width * 0.84, 0, width * 0.5, height);
      ...
      const adColors = [0x00d4ff, 0xff3366, 0xffd700, 0x00e676, 0xff6600, 0xb066ff, 0xffffff];
    ...
    for (let i = 0; i < 5; i++) {
      ...
      lightPool.fillEllipse(width / 2, height * 0.785, width * (0.62 - i * 0.08), height * (0.13 - i * 0.018));
    }
    ...
    for (let i = 0; i < 34; i++) {
      ...
      const mote = this.add.circle(startX, startY, 0.7 + Math.random() * 1.2, 0xfff5cc, 0).setDepth(5);
      this.tweens.add({
        targets: mote,
    ...
    for (let i = 0; i < 90; i++) {
      ...
      foregroundGrass.lineStyle(1, Math.random() < 0.5 ? 0x94e68f : 0x06280f, 0.16);
    ...
    this.goalkeeper = new Goalkeeper(this, width / 2, goalBottomY, keeperHeight);
    ...
    this.statusPanel.fillRoundedRect(width / 2 - statusW / 2, statusY - 12, statusW, 24, 12);
    this.statusPanel.lineStyle(1, 0x9fd7ff, 0.2);
    this.statusPanel.strokeRoundedRect(width / 2 - statusW / 2, statusY - 12, statusW, 24, 12);
  ...
  private createWindHud(controlY: number) {
    ...
    for (let i = 0; i < 4; i++) {
      const gust = this.add.circle(x + 22, y + 43, 2, 0x9fe8ff, 0).setDepth(21);
  ...
  private setWindForShot() {
    const speed = Number(Phaser.Math.FloatBetween(GAME.WIND_MIN_SPEED, GAME.WIND_MAX_SPEED).toFixed(1));
  ...
  private drawWindHud() {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      this.windIcon.lineBetween(endX, yy, endX - dir * 5, yy - 4);
      this.windIcon.lineBetween(endX, yy, endX - dir * 5, yy + 4);
    }
  ...
  private isUiPointer(pointer: Phaser.Input.Pointer): boolean {
    return this.uiTapZones.some((zone) => Phaser.Geom.Rectangle.Contains(zone, pointer.worldX, pointer.worldY));
  }

  private applyWind(direction: AimDirection): AimDirection {
    const drift = this.currentWind.direction * this.currentWind.speed * GAME.WIND_AIM_DRIFT_PER_MPS;
  ...
  private readyForShot() {
    ...
    this.statusText.setText(`SHOT ${this.shotsTaken + 1}/${TOTAL_SHOTS}   ${remaining} LEFT`);
  }
  ...
  private handleShotResult(result: ShotResult) {
    ...
    if (ctx) {
      const durationMs = Math.max(500, Math.round(this.time.now - this.currentShotStartedAt));
      const shotResultKind: 'goal' | 'saved' | 'miss' = result.scored ? 'goal' : result.saved ? 'saved' : 'miss';
      this.shotLog.push({
    ...
    if (dot) {
      const color = result.scored ? (result.bonus ? 0xffd700 : 0x00e676) : result.saved ? 0xff5722 : 0xff1744;
  ...
  private showGoal(result: ShotResult) {
    ...
    const label = result.bonus ? `PERFECT +${result.points}` : `GOAL +${result.points}`;
  ...
  private showSaved() {
    ...
    this.time.delayedCall(1220, () => this.ball.tweenHoldOffsetY(-200, 220, 'Back.Out'));
    this.time.delayedCall(1460, () => this.ball.tweenHoldOffsetY(-128, 220, 'Sine.easeIn'));
  ...
  private showMiss(reason: string) {
    ...
    const style: ToastStyle = reason === 'Over the bar!' ? 'hover' : reason === 'Wide!' ? 'slide' : 'droop';
    this.showToast(label, '#ff1744', style);
    this.time.delayedCall(1700, () => this.readyForShot());
  }

  private showToast(message: string, color: string, style: ToastStyle = 'burst') {
    const { width, height } = this.scale;
    const baseX = width / 2;
    const baseY = height * 0.18;

    this.tweens.killTweensOf(this.toastText);
    this.toastText.setText(message).setColor(color).setAlpha(1).setScale(1).setAngle(0).setPosition(baseX, baseY);
  ...
  private spawnConfetti(bonus: boolean) {
    ...
    for (let i = 0; i < 28; i++) {
      ...
      const piece = this.add.rectangle(x, y, 6, 9, color).setDepth(25).setAngle(Phaser.Math.Between(0, 360));
      this.tweens.add({
        targets: piece,
