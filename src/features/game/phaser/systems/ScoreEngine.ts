...
import type { ShotResult, AimDirection } from '@/features/game/types';
...
export class ScoreEngine {
  ...
  evaluate({
    ...
    // ── Wide miss ─────────────────────────────────────────
    const postLimit = GAME.POST_LIMIT_BASE - difficulty * GAME.POST_LIMIT_REDUCTION;
    ...
    if ((power > 0.95 && timing < 0.3) || (direction.y < -0.85 && timing < 0.4)) {
    ...
    if (timing >= 0.85) {
      result.bonus = true;
      result.points = Math.round(GAME.BASE_GOAL_POINTS * GAME.TIMING_MULTIPLIER + GAME.PERFECT_BONUS);
    ...
    const isControlled = timing >= GAME.ACCURACY_TIMING_GATE && power >= GAME.ACCURACY_POWER_GATE;
